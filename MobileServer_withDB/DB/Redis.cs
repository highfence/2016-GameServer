...
namespace MobileServer_withDB.DB
{
	public static class Redis
	{
		...
		public static ERROR_CODE Init(string address)
		{
			try
			{
				var basicRedisCoonnectString = address.Split(',').ToList();
				var redisSettings = new RedisSettings[basicRedisCoonnectString.Count];

				if (basicRedisCoonnectString.Count > 0)
				{
					var redisSettingsBasic = new RedisSettings[basicRedisCoonnectString.Count];

					for (int i = 0; i < basicRedisCoonnectString.Count; ++i)
						redisSettings[i] = new RedisSettings(basicRedisCoonnectString[i], db: 0);
